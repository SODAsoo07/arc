...
from data_gen.txt_processors.base_text_processor import get_txt_processor_cls
from data_gen.wav_processors.base_processor import get_wav_processor_cls
...
class BasePreprocessor:
    ...
    def process(self):
        ...
        assert len(item_names) == len(
            set(item_names)
        ), "Key `item_name` should be Unique."
        ...
        for item_, (item_id, item) in zip(
            meta_data, multiprocess_run_tqdm(process_item, args, desc="Preprocess")
        ):
            if item is not None:
                item_.update(item)
                item = item_
        ...
        # add encoded tokens
        ph_encoder, word_encoder = self._phone_encoder(phone_list), self._word_encoder(
            word_list
        )
        ...
        for idx, item_new_kv in multiprocess_run_tqdm(
        ...
        with open(f"{processed_dir}/{self.meta_csv_filename}.json", "w") as f:
            f.write(
                re.sub(
                    r"\n\s+([\d+\]])",
                    r"\1",
                    json.dumps(items, ensure_ascii=False, sort_keys=False, indent=1),
                )
            )
        remove_file(wav_processed_tmp_dir)

    @classmethod
    def preprocess_first_pass(
        cls,
        item_name,
        txt_raw,
        txt_processor,
        ...
        try:
            ...
            ph, txt, word, ph2word, ph_gb_word = cls.txt_to_ph(
            ...
            move_link_func = (
                move_file if os.path.dirname(wav_fn) == wav_processed_tmp else link_file
            )
    ...
    @staticmethod
    def process_wav(
        item_name, wav_fn, processed_dir, wav_processed_tmp, preprocess_args
    ):
        processors = [
            get_wav_processor_cls(v) for v in preprocess_args["wav_processors"]
        ]
    ...
    def _word_encoder(self, word_set):
        ...
        if self.preprocess_args["reset_word_dict"]:
            ...
            print(
                f"| Build word set. Size: {len(word_set)}, #total words: {total_words},"
    ...
    @classmethod
    def preprocess_second_pass(
        cls, word, ph, spk_name, word_encoder, ph_encoder, spk_map
    ):
